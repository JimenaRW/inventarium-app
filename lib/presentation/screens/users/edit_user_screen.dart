...
class _EditUserScreenState extends ConsumerState<EditUserScreen> {
  ...
  Future<void> _confirmAndUpdate() async {
    if (selectedRole == widget.user.role && selectedStatus == widget.user.estado) {
        ...
        content: Column(
          ...
          children: [
            ...
            if (selectedStatus != widget.user.estado)
              Text('• Cambiar estado a ${selectedStatus == 'active' ? 'Activo' : 'Inactivo'}'),
    ...
    try {
      if (selectedRole != widget.user.role) {
        await ref.read(userNotifierProvider.notifier).updateUserRole(
          widget.user.id,
          selectedRole,
        );
      }
      ...
      if (selectedStatus != widget.user.estado) {
        await ref.read(userNotifierProvider.notifier).updateUserStatus(
          widget.user.id,
          selectedStatus,
        );
      }
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // Sección Estado con RadioButtons
            const Text("Estado:", 
              style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
            const SizedBox(height: 8),
            Column(
